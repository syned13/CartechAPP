...
class OrdersList extends StatelessWidget{
  ...
  Widget _orderList(List<ServiceOrder> orders){
      ...
      cards.add(Card(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              Text(_getDateFromDate(orders[i].createdAt), style: Resources.WhiteTextStyle,),
              Text(_getHourFromDate(orders[i].createdAt), style: Resources.WhiteTextStyle),
              SizedBox(height: 20,),
              Text("Estado de la orden: " + _parseStatus(orders[i].status), style: Resources.WhiteTextStyle,),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: StreamBuilder<OrdersListState>(
        stream: this.orderListType == OrderListType.Past ? _bloc.currentOrdersStream : _bloc.pastOrdersStream,
        builder: (context, snapshot){
          ...
          if(snapshot.data is OrdersListStateError){
            ...
            return Center(child: Text(errorMessage, style: TextStyle(color: Colors.red[300]),));
          }
          ...
          if(snapshot.data is OrderListStateDone){
            ...
            if(orders.length == 0){
              return Center(child: Text("Todavia no tienes ordenes"),);
            }
