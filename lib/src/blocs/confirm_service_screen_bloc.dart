...
class ConfirmServiceScreenBloc extends Bloc{
  ...
  Stream<ConfirmServiceState> get confirmServiceStream  => _controller.stream.asBroadcastStream();
  ...
  void confirmServiceRequest() async{
    ...
    bool success = true;
    String response = await ApiClient.post(serviceRequest.toJSON(), "/order").catchError( (error){
      String errorMessage = error.toString();
      developer.log("error_message_confirm_service_order: " + errorMessage);
      if(errorMessage == "internal server error"){
        errorMessage = "Ha ocurrido un error";
      }
      else if(errorMessage == "user is not allowed to have more than one service"){
